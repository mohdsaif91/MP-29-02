...
import Modal.AllDataPojoClass;
import Modal.cartHelperPojo;
...
//    List<adminordertable> alist = new ArrayList<>();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (tablename.equals("review")) {
            ...
            System.out.println("ye call hota ki nai");
            Gson gs = new Gson();
            ...
            try {
                ...
                String query = "insert into PaymentPojo(id,cardholdename,cardnumber,cvvnumber,emailid,expirydate,mobilenum,paymentoption)"
                        + "values('" + orderid + "','" + cardHoldeName + "','" + cardNumber + "'," + cvvNumber + ",'" + emailid + "','" + expiryDate + "'"
                        + ",'" + mobilenum + "','" + tablename + "')";
...
//        -------------------------------------------------------------------------
            ...
            System.out.println(fname + "" + lname + "" + uname + "" + address1 + "" + address2 + "" + country + "" + state + "" + zipcode + "" + rcode
                    + "" + genericname + "" + productprice + " " + date);
            String query = "INSERT INTO adminordertable("
                    + "            id, address1, address2, brandname, city, companyname, country,"
                    + "            firstname, genericname, imagename, lastname, mobilenum, packaging,"
                    + "            paymentdoen, productid, productprice, quantity, state,"
                    + "            username, yyyy_mm_dd, zipcode)"
                    + "    VALUES ('" + orderid + "', '" + address1 + "','" + address2 + "','" + brandname + "','" + city + "','" + tablename + "','" + country + "',"
                    + "            '" + fname + "', '" + genericname + "','" + imagename + "','" + lname + "','" + mobilenum + "','" + packaging + "',"
                    + "            'no', '" + productid + "', '" + productprice + "','" + quantity + "','" + state + "' , "
                    + "            '" + uname + "', '" + date + "', '" + zipcode + "');";
            System.out.println(query);
            con = ConnectionClass.getConnection();
            ps = con.prepareStatement(query);
            ...
            System.out.println("/////////////798789789789879987789879897879879");
            updatecart(chplist);

            //        alist.add(ao);
            List<orderpojo> list = (List) request.getAttribute("productdetials");
//        System.out.println("Size  is--" + list.size());
//        list.forEach((orderpojo o) -> {
//            System.out.println(o.getBrandname() + " ============");
        ...
        try {
            ...
            if (cartList.isEmpty()) {
                ...
                String query = "insert into " + username + "(productid,companyname,price,packaging,quantity,productname,show,image)"
                        + "values('" + id + "','" + tablename + "','" + price + "','" + pqckaging + "','" + quantity + "','" + productname + "','order','" + cartimagename + "')";
                System.out.println(query);
                con = ConnectionClass.getConnection();
                ps = con.prepareStatement(query);
                ps.executeUpdate();
                ...
                System.out.println(cartList.size() + " ******45645646565655");
